...
int num2 = 0;
char flag=0;
float ratio = 1, Check, Check2, Check3;
...
void APP_Tasks ( void )
{
  ...
  if (PORTBbits.RB13 == 1 && prevuser == 0) {
    // button was let go, start timer
    mode = 1;
  }
  else if (PORTBbits.RB13 == 0) {
    // button is depressed, reset timer
    mode = 0;
    time = 0;
  }

  if (mode == 1 && time > 500) {
    // time is counting, check analog for blocking
    a = readADC();
    if (a < 600) {
      // there is a block, stop the timer
      mode = 0;
    }
  }

  prevuser = PORTBbits.RB13;
        ...
        case APP_STATE_CHECK_CDC_READ:
            ...
            if(appData.isReadComplete == true)
            {
                ...
                // otherwise put the char in a buffer and increment the index
                    ...
                    memcpy(&(rxbuffer[0]),&(appData.readBuffer[0]),16);
                }
            }
            
            Check2 = (rxbuffer[9]+rxbuffer[10]+rxbuffer[11]+rxbuffer[12])/4;
            Check = (rxbuffer[1]+rxbuffer[2]+rxbuffer[3]+rxbuffer[4]+rxbuffer[5])/5;
